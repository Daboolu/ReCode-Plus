...
import type { TranslationKeys } from "@/constants/languages";
...
import { Code, FileText, Calendar, Edit3, ExternalLink } from "lucide-react";
import { TimelineSection } from "@/components/questions/editor/TimelineSection";
...
export const QuestionPreviewModal = ({
  ...
  const timelineEvents = getTimelineEvents(data);
          ...
          <div className="flex items-start justify-between gap-6">
            <div className="space-y-2 flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                >
                  {t(
                    `questionsTable.diff${data.problem.difficulty}` as TranslationKeys
                  )}
                </Badge>
                {/* mastery */}
...
function getTimelineEvents(data: QuestionRowData): ReviewTimelineEvent[] {
  const persistedEvents = data.reviewEvents ?? [];
  const events: ReviewTimelineEvent[] = [...persistedEvents];

  if (!persistedEvents.some((event) => event.type === "created")) {
    events.push({
      id: `${data.id}-created`,
      type: "created",
      rating: null,
      masteryBefore: null,
      masteryAfter: data.masteryLevel,
      intervalBefore: null,
      intervalAfter: null,
      easinessBefore: null,
      easinessAfter: null,
  ...
  if (
    ...
    events.push({
      id: `${data.id}-last-review`,
      type: "reviewed",
      rating: data.masteryLevel,
      masteryBefore: null,
      masteryAfter: data.masteryLevel,
      intervalBefore: null,
      intervalAfter: null,
      ...
      easinessAfter: null,
      note: "Latest review",
      createdAt: toIsoString(data.lastReview),
    });
  }
