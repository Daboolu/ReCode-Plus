      ...
      if (!existingProgress) {
        await tx.reviewEvent.create({
          data: {
            ...
            type: "created",
            masteryAfter: formData.masteryLevel,
            intervalAfter: 1,
            easinessAfter: getInitialEasiness(formData.difficulty),
            note: "Problem added",
          },
        });
      }
      ...
      if (formData.code && formData.code.trim() !== "") {
        ...
        await tx.reviewEvent.create({
          data: {
            ...
            type: "submission",
            masteryBefore:
              existingProgress?.masteryLevel ?? progress.masteryLevel,
            masteryAfter: progress.masteryLevel,
            intervalBefore: existingProgress?.interval ?? progress.interval,
            intervalAfter: progress.interval,
            easinessBefore: existingProgress?.easiness ?? progress.easiness,
            easinessAfter: progress.easiness,
            note: `${formData.language} solution ${existingSubmission ? "updated" : "saved"}`,
          },
        });
      }
    });
