# RecallAgent — 拥有长期记忆的编程复习 Agent

[English](./README.md)

RecallAgent 是一个面向刻意练习的编程复习 Agent。它会读取当前题目、提交代码、历史复习记录和 Note 记忆，规划安全的函数调用，在本机执行代码，解释真实输出，并把有价值的结论带入下一次复习。
...
- **Agent 自动复习闭环**：读取题目和代码、生成调用、校验参数、执行测试、解释真实输出，并把结果记录为记忆。
- **持久化 Note 记忆**：按照代码版本保存真实调用与分析，下次对话不再从零开始。
- **本地或远程推理**：默认使用本机 Ollama，也可以配置 OpenAI-compatible 远程 API。
...
1. 点击复习建议气泡，或手动选择一道题。
2. RecallAgent 读取题目、当前代码、复习历史和 Note 记忆。
3. 提交代码后，模型识别可调用的函数或类方法，并生成仅包含 JSON 数据的参数。
4. 服务端校验调用目标，在受限的本地执行器中运行用户提交的原始代码。
5. 真实输出返回 Agent 进行解释；模型生成的探测只作为观察，不冒充在线判题结果。
6. 本轮调用、输出和判断按照代码版本写入 Note，供下次交互使用。
7. 前端在聊天框中流式显示规划、校验、执行、判断和写入记忆等阶段。
...
完成常规 Prisma 初始化后，在另一个终端启动 RecallAgent：
